...
def combine_with_neighbors_attenuation(
    user_cart, users_carts, neighbors, alpha, k_nearest
):
...
## TIFU KNN-based recommendations


def train_tifu(
    items_mapping, purchase_data, users_as_vectors, login, new_basket, model_params
):
    """
    Build recommendations using the TIFU KNN algorithm
    """
    purchase_data = pd.read_csv(cfg.purchase_data)
    purchase_data.rename(columns={"order_completed_at": "time"}, inplace=True)
    purchase_data["time"] = pd.to_datetime(
        purchase_data["time"], format="%Y-%m-%d %H:%M:%S"
    )

    with open(cfg.users_as_vectors, "rb") as data:
        users_as_vectors = pickle.load(data)

    # Select purchase history for the specific client
    purchase_data = purchase_data[purchase_data["user_id"] == int(login)].copy()

    # Add new basket to purchase history
    new_purc = []
    ...
    if int(login) in range(20000):
        ...
        purchase_data = pd.DataFrame(
            {"cart": new_purc, "user_id": 20000, "time": dt.datetime.today()}
        )
    ...
    if len(purchase_data) > 0:
        ...
        # Update the user's weighted average basket
        if int(login) in range(20000):
    ...
    # Find the nearest neighbors for the client
    model = NearestNeighbors()
    model.fit(users_as_vectors)
    neighbors = model.kneighbors(
        user_as_vector, n_neighbors=model_params["k_nearest"] + 1, return_distance=False
    )

    # Combine the user's weighted basket with those of their nearest neighbors
    user_as_final_vector = combine_with_neighbors_attenuation(
        user_as_vector,
        users_as_vectors,
        neighbors,
        model_params["alpha"],
    ...
    products = (
        user_as_final_vector.sort_values(ascending=False)
        .head(model_params["top_k"])
        .index.tolist()
    )
...
def train_popular(items_mapping, purchase_data, login, new_basket, model_params):
    """
    Build recommendations based on the most popular items
    """
    purchase_data = pd.read_csv(cfg.purchase_data)
    purchase_data.rename(columns={"order_completed_at": "time"}, inplace=True)
    purchase_data["time"] = pd.to_datetime(
    ...
    # Product recommendations
    products = (
        purchase_data.groupby("cart")["user_id"]
        .count()
        .to_frame()
        .reset_index()
        .sort_values("user_id", ascending=False)
        .head(model_params["top_k"])["cart"]
        .tolist()
    )
