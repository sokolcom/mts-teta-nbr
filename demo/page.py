...
from train_model import train_popular, train_tifu
...
def welcome():
    ...
    st.markdown(
        "Here you can check out our `Next Basket Recommendations` and play around with it!"
    )
    st.markdown(
        """
        * :bulb: Pick a model and set its hyper-parameters
        * :dizzy: Train it and check its performance metrics
        * :shopping_trolley: Create *your own* basket and get *your personal* recommendations
        -----
    """
    )
...
def render_top(items_mapping, model_type):
    ...
    with st.container():
        ...
        with layout[0].form(key="login"):
            ...
            btn_submit_login = st.form_submit_button(label="Send")
        # New basket
        with layout[1].form(key="create_basket"):
            new_basket = st.multiselect(
                "Select items", items_title, items_title[: cfg.DEFAULT_ITEMS_AMOUNT]
            )
            btn_submit_basket = st.form_submit_button(label="Create basket")

        # Display metrics for different models
...
def render_mid(items_mapping, login, new_basket, model_type, model_params):
    ...
    with st.container():
        ...
        if model_type == cfg.MODEL_TIFUKNN:
            ...
            items_ids = train_popular(
                items_mapping, cfg.purchase_data, login, new_basket, model_params
            )
